...
public class Trains {
    ...
    public Trains() {
        ...
        try {
            json = RestCalls.sendGET("http://localhost:8189/train");
    ...
    public List<Train> update() throws IOException {
        json = RestCalls.sendGET("http://localhost:8189/train");
        trains = objectMapper.readValue(json, new TypeReference<List<Train>>(){});
        return trains;
    }
    ...
    public String sellTicket(String id) throws IOException {
        if(RestCalls.sendPUT("http://localhost:8189/reservation/" + id)) {
            return "OK";
        }
        return "KO";
    }

    // Iterate through local list of train tickets
